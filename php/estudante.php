...
if(isset($_POST['cadastrarEstudante'])){
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" && empty($telefone_erro)){
		...
		$cep = str_replace($simbolos,"",$raw_cep);
	...
	if(!isset($json->erro)){
		...
		$conteudo='<tr>
						<td><input type="text" name="telefone" id="telefone" placeholder="telefone" value="'.$telefone.'" autocomplete="off"
		...
		$exige = "required";
		$exigeNome = 'pattern="([A-Za-z ]{20,50})" required';
	}else{
		$cep = "";
		echo '<script language="javascript" type="text/javascript">
			function exibirErro(){
